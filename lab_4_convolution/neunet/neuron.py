...
class Neuron:
    ...
    def activate(self, inp: float = None) -> t.Union[float, type(np.nan)]:
        if inp is not None:
            tmp = self.__activation_func(inp)
            self.__output = tmp
            return tmp
    ...
    def derivative_from_input(self, precision: float, inp: float = None) -> t.Union[float, type(np.nan)]:
        if inp is not None:
            return nf.derivative(self.__activation_func, inp, precision)
        if not np.isnan(self.__input):
